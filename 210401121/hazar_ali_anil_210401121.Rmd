...
```{r echo=FALSE, message=FALSE, warning=FALSE}
# Veri setindeki eksik değerleri doldur (örneğin, ortalama notlar)
merged_data[is.na(merged_data)] <- 0

# Gruplandırma ve filtreleme işlemleri
# Benzer çalışma sürelerine sahip öğrencileri filtreleyip, adres türü, internet erişimi ve ağırlıklı ortalamaya göre gruplandırıyoruz.
# Daha sonra, her grup için ortalama seyahat süresini hesaplıyoruz.
...
# Tablo 1: Adres Türüne ve Internet Erişimine Göre Benzer Çalışma Sürelerine Sahip Fakat Farklı Notlar Alan Öğrencilerin İncelenmesi
# Bu tablo, benzer çalışma sürelerine sahip ancak farklı notlar alan öğrencilerin adres türüne ve internet erişim durumlarına göre incelenmesini göstermektedir.
cat("Benzer Çalışma Sürelerine Sahip Ancak Farklı Sınıflarda Okuyan Öğrencilerin Adres Türü ve İnternet Erişimine  Göre İncelenmesi\n")
head(head(similar_study_time_groups))

# Grafik 1: Benzer çalışma sürelerine sahip kırsal ve kentsel öğrencilerin ortalama seyahat süreleri karşılaştırılması
### Bu grafik, benzer çalışma sürelerine sahip olan ancak farklı adres türlerinde yaşayan öğrencilerin ortalama seyahat sürelerini karşılaştırır.
...
# Grafik 2: Adres Türüne ve Internet Erişimine Göre Benzer Çalışma Sürelerine Sahip Fakat Farklı Notlar Alan Öğrencilerin Internet Erişim Durumları Karşılaştırılması
# Bu grafik, benzer çalışma sürelerine sahip ancak farklı notlar alan öğrencilerin adres türüne ve internet erişim durumlarına göre dağılımını gösterir.
...
-Bu projede, kırsal ve kentsel alanlarda yaşayan öğrencilerin akademik performansları arasındaki farklılıkları detaylı bir şekilde analiz ettik. Amacımız, çevik prensiplere uygun bir yaklaşımla, bu farklılıkların temel nedenlerini belirleyerek hükümete etkili çözüm önerileri sunmaktı.

-Q4 aşamasının ardından projeden sorumlu hükümet görevlileri ile gerçekleştirdiğimiz bir görüşme sonucunda, eğitmen ve psikologların ortak kararı doğrultusunda öğrencilerin başarılarını ölçerken sadece sayısal bir dersin değil, aynı zamanda sözel bir dersin verilerini de dahil etmemiz gerektiği belirlendi. Bu karar, öğrencilerin başarılarını daha kapsamlı bir şekilde değerlendirmemize olanak tanıdı.

-Çevik prensiplere uygun bir biçimde yönetilen projemize yeni bir veri seti eklemek, esnek ve hızlı bir şekilde uygulanabilen çevik yazılım yaklaşımımızla oldukça uyumlu oldu. Bu yeni veri seti, sadece sayısal bir perspektifle değil, aynı zamanda sözel yetenekleri de içererek daha geniş bir bakış açısı sunmamıza olanak sağladı.

-Bu gelişme, projemizin daha kapsamlı ve çeşitli bir analiz yapabilme yeteneğini artırdı. Artık sadece matematik dersi üzerinden değil, aynı zamanda sözel bir dersin verileri üzerinden de öğrenci performansını değerlendirebiliyoruz. Bu, elde ettiğimiz sonuçların daha geniş bir perspektifle değerlendirilmesine ve çözüm önerilerinin daha etkili bir şekilde geliştirilmesine olanak tanıdı.

-Proje kapsamında, öğrencilerin Portekizce ve Matematik derslerindeki not ortalamalarını kullanarak %30 vize ve %40 final ağırlıklı bir karşılaştırma gerçekleştirdik. Çalışma süreleri benzer olan öğrencileri seçerek adil bir karşılaştırma yapmaya özen gösterdik.

-Elde ettiğimiz sonuçlar, öğrencilerin akademik performansı üzerinde internet erişiminin belirgin bir etkisi olmadığını, ancak eğitime ulaşım süresinin önemli bir faktör olduğunu gösterdi. Bu durum eğitimde fırsat eşitliğinin sağlanması için çok kritik bir rol oynuyor.

-Çevik prensiplere uygun olarak, projemizi hızlı ve etkili bir şekilde yönettik. Elde ettiğimiz verileri kullanarak çözüm odaklı analizler gerçekleştirdik. Sonuçlar, kırsal alanda yaşayan öğrencilerin eğitime ulaşım sorunlarına çözüm bulunması gerektiğini ortaya koydu. Bu, çevik bir yaklaşımla hızlı bir şekilde uygulanabilecek ve maliyeti düşük bir çözüm olarak öne çıkıyor. Bu noktada, hükümetin bu soruna yönelik adımlar atması önem arz etmektedir.

-Projemizin güçlü yönü, geniş veri seti üzerinde yapılan detaylı analizdir. Ancak, sınırlı sayıda dersin notlarını içermesi projenin zayıf yönü olarak göze çarpıyor. Gelecekte, daha kapsamlı bir analiz için daha fazla dersin dahil edilmesi ve diğer faktörlerin (örneğin, öğretmen kalitesi, okul donanımı) incelenmesi önerilmektedir."

------------------------------------------------------------------------------------------------------------------------


```{r test_chunk, echo=FALSE, message=FALSE, warning=FALSE, include=FALSE}
testthat::test_file("tests/Q6_tests_ali_anil_hazar_210401121.R")
```